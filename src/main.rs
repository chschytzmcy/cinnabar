...
fn print_partial(text: &str, tty: bool) {
    let compact = itn(&strip_display_spaces(text));
    if tty {
...
/// - "百分之N"：→ "N%"（作为整体识别，不能被拆）
///
/// **不**覆盖（v1 跳过，留给未来或更全的库）：
/// - 复杂小数（零点一五、负数、分数）
/// - 序数（第一 → #1）
/// - 复杂年份/日期格式
/// - 与阿拉伯数字混排
///
/// 设计取舍：不引入 regex 依赖，手写解析器。覆盖 80% 实际用例，
/// 边界情况（"两三个"这种口语范围）保持原样不强行转换。
fn itn(text: &str) -> String {
    if text.is_empty() {
