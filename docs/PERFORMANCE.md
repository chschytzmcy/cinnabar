...
| 指标 | CLI 模式 | GUI 模式 | 目标 |
|------|---------|---------|------|
| 端到端延迟（流式 P50） | 65ms | 72ms | <100ms |
| 端到端延迟（流式 P95） | 95ms | 108ms | <150ms |
| 端到端延迟（流式 P99） | 125ms | 142ms | <200ms |
| **非流式 refine 延迟** | ~125–500ms | ~125–500ms | <500ms |
| 内存占用（仅流式） | 85MB | 102MB | <200MB |
| 内存占用（含非流式） | ~470MB | ~490MB | <600MB |
| CPU 使用率（单核） | 28% | 35% | <50% |
| 启动时间 | 1.2s | 1.8s | <3s |
| 模型加载时间（仅流式） | 0.8s | 0.8s | <2s |
| 模型加载时间（含非流式） | ~3.5s | ~3.5s | <5s |

### 延迟分解

```
流式阶段 (65ms) = 音频捕获 (10ms) + 重采样 (1ms) +
                  通道传输 (2ms) + 流式 ASR 推理 (48ms) +
                  显示输出 (4ms)

非流式 refine（v1.2.5+，切段后） = 离线 ASR 推理 (~125–500ms)
```

> 非流式 RTF（官方文档）为 0.062，对 2–3 秒的 segment 意味着 ~124–186ms
> 额外延迟（按官方基准配置；CPU 不同时会浮动）。Greedy 解码，单线程也够用。

**为什么有两段？**
- **流式 partial**：每 ~100ms 推一次，让用户看到"打字机"效果，跟手
- **非流式 refine**：切段后另起一份离线 ASR 整段重算，CER 比流式低 2-5 个
  百分点，立即覆盖屏幕上的 `✅ 流式文本`

**如果不想等 refine**：在 `config.toml` 设 `enable_offline_refine = false`，
或 CLI 加 `--no-offline-refine`，退回纯流式。

### 资源消耗

**磁盘空间**：
- 二进制文件: 8.2MB (release)
- 流式 ASR + VAD 模型: ~42MB (INT8)
- 非流式 ASR 模型（v1.2.5+）: ~355MB
- **总计（仅流式）**: ~50MB
- **总计（含非流式）**: ~400MB
